...
- 第一遍必读：第0～6、8、10、12～15节，理解 paired、目录、baseline 和排错；
- 动手主线：第10节完整执行流程；
- 第二遍深入：第16～17节，阅读本机 SIDD 记录和 Dataset Card；
- 历史结果只用于理解证据，不要把已有产物当成自己完成练习的证明。
...
```text
stage2_ai_isp/datasets/sidd_tiny/
  train/
    noisy/
      pair_00001.png
      pair_00002.png
...
```text
stage2_ai_isp/scripts/04_prepare_paired_rgb_subset.py
```
...
```text
stage2_ai_isp/scripts/06_inspect_paired_dataset.py
```
...
```text
stage2_ai_isp/configs/paired_rgb_sidd_tiny_dncnn_l2.yaml
```
...
```text
stage2_ai_isp/datasets/sidd_tiny/train/noisy
stage2_ai_isp/datasets/sidd_tiny/train/clean
stage2_ai_isp/datasets/sidd_tiny/val/noisy
stage2_ai_isp/datasets/sidd_tiny/val/clean
```
...
```text
stage2_ai_isp/scripts/14_export_week2_dataset_card.py
```
...
```text
stage2_ai_isp/reports/figures/week2_dataset_card/week2_dataset_card.csv
stage2_ai_isp/reports/figures/week2_dataset_card/week2_dataset_card.md
```
...
```text
config,noise,patch_size,val_size,input_psnr,input_ssim
stage2_ai_isp\configs\paired_rgb_sidd_tiny_dncnn_l2.yaml,real_paired_rgb,128,200,26.7302,0.52412
```

...
- 第一遍必读：第0～3、8～12节，先掌握输入输出、实验纪律、metrics 和验收；
- 动手主线：第4～7节，每轮只改变一个变量；
- 第二遍深入：第14～16节，复查本机短训、标准版和自动模型对比卡；
- 历史 run 的 loss、步数和宽度不完全统一，只能按对应协议解释，不能推成普遍模型排名。
...
```text
stage2_ai_isp/configs/paired_rgb_sidd_tiny_dncnn_l2_patch64_2000.yaml
```
...
```text
stage2_ai_isp/runs/paired_rgb_sidd_tiny_dncnn_l2_300
stage2_ai_isp/runs/paired_rgb_sidd_tiny_unet_l1_300
stage2_ai_isp/runs/paired_rgb_sidd_tiny_nafnet_lite_l1_300
```
...
```text
stage2_ai_isp/scripts/15_export_week3_model_comparison.py
```
...
```text
stage2_ai_isp/reports/figures/week3_model_comparison/week3_model_comparison.csv
stage2_ai_isp/reports/figures/week3_model_comparison/week3_model_comparison.md
```

...
| 名词 | 它代表什么 | 为什么需要 | 错误表现与验证 |
|---|---|---|---|
| 线性 RGB | 码值与场景光强近似成比例的三通道数据 | AWB gain、CCM 矩阵和多数物理量分析依赖线性关系 | 若已经做过 Gamma 却当线性数据处理，矩阵和亮度比例失去物理意义；用中间结果和曲线确认编码状态 |
| Linear display | 直接把线性 RGB 缩放成预览 | 用来观察中间结构和数值，不是最终显示标准 | 画面通常偏暗，不能据此断言曝光失败；应和相同数据的 OETF 后预览配对 |
| CCM | 用 `3x3` 矩阵把 camera RGB 映射向目标颜色空间 | AWB 只修白点，不能修传感器通道串扰和色相关系 | 矩阵方向/白点错会系统性偏色并产生负值或 clip；用 identity、单像素手算、色卡或已知颜色验证 |
| Tone Mapping | 将场景或线性 RGB 的动态范围映射到有限显示范围 | 高光和暗部无法直接同时装入输出范围 | 曲线太强会发灰、光晕或丢局部对比；检查曲线、亮度百分位和固定 ROI，不只看平均亮度 |
| Gamma / sRGB OETF | 将线性显示值编码为非线性输出码值 | 匹配显示和存储编码，提高暗部码值利用率 | 重复编码会泛白，漏编码会偏暗；用 0、断点、中灰、1 的曲线点和往返测试验证 |

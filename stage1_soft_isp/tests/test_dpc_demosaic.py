...
# ????? DpcAndDemosaicTests ??????? test_dpc_demosaic ?????????
class DpcAndDemosaicTests(unittest.TestCase):
    ...
    # ????? test_injected_hot_pixel_is_detected_and_repaired ???????? ISP ????????????
    ...
    # ????? test_constant_bayer_remains_constant_after_demosaic ???????? ISP ????????????
    ...
    # ????? test_sampled_values_are_preserved ???????? ISP ????????????

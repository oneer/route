...
## 12. 深度补强：从全图指标走向局部 IQA

Week5 已经证明全图 PSNR/SSIM/MAD 可以帮助观察趋势，但它们对 ISP 问题的解释力有限。下一步评价体系应拆成三层：
...
|---|---|
| center | 整体颜色、主体结构 |
| corner | LSC、边缘噪声、角落亮度 |
| dark | 暗部噪声、黑位、色偏 |
| highlight | clipping、高光压缩 |
| texture | demosaic、锐度、假彩 |
| skin | 肤色自然度 |
| sky | AWB、banding、色偏 |
...
|---|---:|---:|---:|---:|---|---|
| dark | 待填写 | 待填写 | 待填写 | 待填写 | noise / color_shift | 待填写 |

### 12.2 主观评价量表

主观评价不应该只写“好看/不好看”，而要拆成维度：

| 维度 | 评分关注 | 建议权重 |
|---|---|---:|
| 颜色准确性 | 中性灰、肤色、天空是否自然 | 30% |
| 细节清晰度 | 边缘、纹理、文字是否糊 | 25% |
| 噪声控制 | 暗部是否干净，是否有彩噪 | 20% |
| 伪影抑制 | 假彩、拉链、摩尔纹、halo | 15% |
| 整体观感 | 是否自然，是否过度处理 | 10% |

这样做的价值是：当 PSNR 和主观观感冲突时，可以说清楚冲突来自哪个维度。

### 12.3 参数敏感性分析

消融实验只回答“模块开或关”。产品级 tuning 更关心“参数改一点会怎样”。

建议优先扫描：

| 参数 | 取值 | 观察 |
|---|---|---|
| DPC `mad_k` | 1.0 / 2.0 / 3.0 / 4.0 | 检测点数、误检、坏点 crop |
| LSC strength | 0.5 / 0.75 / 1.0 | 角落亮度、噪声放大 |
| Tone strength | 多档 Reinhard / S-curve | 高光和中间调 |
| AWB gray threshold | 多档低饱和筛选 | gain 稳定性和失败场景 |

参数敏感性表能回答：

```text
当前参数是稳健的，还是刚好在某几张图上有效？
```

### 12.4 rawpy reference 的正确用法

rawpy 可以作为成熟 pipeline 参考，但不能被写成 ground truth。

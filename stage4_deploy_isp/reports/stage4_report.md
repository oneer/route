...
→ ORT CUDA I/O Binding device input/output
```

阶段三的 global Reinhard 学习节点已通过固定 manifest 串到 Stage 4 C++ ORT runner，但完整 Bayer RAW ISP、真实 Camera buffer 和 Android 真机仍未接入。因此完整故事仍应表述为“带 C++ ISP 学习桥接的 RGB AI denoise 部署闭环”，不能包装成 sensor RAW ISP 或移动端量产部署。
...
| ORT CUDA I/O Binding | FP32 | ORT CPU | max `2.68e-7`，quality `32.98396 dB` | device input/output 已复验；preprocess 仍在 CPU |
...
- ORT CUDA I/O Binding：device input/output、`1 H2D / 0 intermediate D2H / 1 final D2H`；inference p50/p90 `3.296/3.754 ms`，host-to-final-host e2e p50/p90 `10.477/11.047 ms`，sampled peak RAM `614.61 MiB`。
...
| Week 6 | kernel/profile 与 ORT CUDA device input/output 已验证；custom CUDA preprocess 直绑仍未完成 |

## 当前未复验/未完成

- Android/ARM、NCNN/MNN、功耗、温度、移动端内存；
- 阶段三 C++ BLC/LSC/tone mapping 串联；
- custom CUDA preprocess pointer/stream → ORT input 的直接绑定；
- pinned/pageable 与 CUDA Event 代码已有，可信收益实测和 Nsight Systems/Compute 时间线未完成；
- TensorRT C++ runner、dynamic profile、TensorRT INT8；
- 更大且有场景标签的 calibration/test set。

## 下一步最值得做的三项

1. 把 custom CUDA preprocess device output 直接绑定到 ORT CUDA/TensorRT 输入，消除 preprocess 后的 host staging，并用 CUDA event/Nsight 复核时间线。
2. 扩大并标注 calibration/evaluation 数据，补暗部、高光、纹理、颜色分组和 TensorRT INT8。
3. 若目标是高通手机影像岗位，先用 NCNN/ORT Mobile 建立 vendor-neutral Android baseline，再用 QAIRT/QNN 在 Snapdragon 真机验证 CPU/GPU/HTP 的 raw tensor 对齐、实际 graph partition、冷/热 p50/p90、内存、温度和功耗；在没有真机与工具链证据前保持 `not_run`。
